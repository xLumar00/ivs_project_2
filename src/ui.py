...
def key_pressed(event):
    ...
    elif keysym == 'Return' or keysym == 'KP_Enter' or char == '=':
        button_equals()
...
def button_factorial():
    ...
    if last_number != "":
        try:
            raw_val = float(last_number)
            if not raw_val.is_integer():
                raise ValueError("Only integers for !")
...
help_button = ctk.CTkButton(app, text="?", width=15, height=15, font=("Roboto", 14, "bold"), fg_color="#7a7a7a",
                            hover_color="#333333", command=open_help)
